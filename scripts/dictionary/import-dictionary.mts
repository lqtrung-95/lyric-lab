...
const sinoViet = [...parseUnihanVietnamese(readFileSync(`${DIR}/Unihan_Readings.txt`, "utf8"))].map(([hanzi, readings]) => ({ hanzi, readings }));

const byLevel: Record<number, number> = {};
words.forEach((w) => w.hsk_level && (byLevel[w.hsk_level] = (byLevel[w.hsk_level] ?? 0) + 1));
console.log(`CEDICT ${cedict.length} dòng, HSK 3.0 ${hsk.length} từ → ${words.length} mục; Hán Việt ${sinoViet.length} chữ`);
