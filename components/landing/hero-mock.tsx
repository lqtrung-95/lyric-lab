...
const LYRICS = [
  { zh: "窗外的城市慢慢睡了", py: "chuāng wài de chéngshì mànmàn shuì le", current: false },
  { zh: "我从来没想过会离开", py: "wǒ cónglái méi xiǎng guò huì líkāi", current: true },
  { zh: "你的笑比星光还亮", py: "nǐ de xiào bǐ xīngguāng hái liàng", current: false },
];
...
export function HeroMock() {
  return (
    <div aria-hidden="true" className="relative mx-auto w-full max-w-md select-none lg:max-w-none">
      ...
      <div className="relative rotate-[1.5deg] rounded-3xl bg-surface-container-lowest p-6 shadow-[0_24px_60px_-20px_rgba(60,30,20,0.35)] ring-1 ring-outline-variant/40">
      ...
      <div className="relative -mt-6 ml-6 -rotate-[1deg] rounded-3xl bg-inverse-surface p-5 text-inverse-on-surface shadow-[0_24px_60px_-24px_rgba(20,10,5,0.6)] sm:ml-12">
        ...
        <ul className="space-y-2.5">
          {LYRICS.map((l) => (
            <li key={l.zh} className={l.current ? "" : "opacity-70"}>
              <p className="text-label-sm">{l.py}</p>
              <p lang="zh" className={`font-serif ${l.current ? "text-[24px]" : "text-[18px]"}`}>{l.zh}</p>
            </li>
          ))}
        </ul>
      </div>
